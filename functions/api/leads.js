...
async function createLead({ request, env }) {
  ...
  // Minimum loan size: we don't take enquiries under $15,000. Drop silently.
  const amount = toInt(body.loanAmount);
  if (amount != null && amount < 15000) { return json({ ok: true }); }
  ...
  lead.details = JSON.stringify({
    ...
    currentRate: str(body.currentRate, 20) || null,
    currentRepayment: str(body.currentRepayment, 20) || null,
    repaymentFreq: str(body.repaymentFreq, 20) || null,
...
async function emailLead(env, lead) {
  ...
  const lines = [
    ...
    "Current rate: " + (x.currentRate ? x.currentRate + "%" : "—"),
    "Current repay:" + (x.currentRepayment ? "$" + x.currentRepayment + (x.repaymentFreq ? " " + x.repaymentFreq.toLowerCase() : "") : "—"),
    "Use:          " + (lead.use_type || "—"),
